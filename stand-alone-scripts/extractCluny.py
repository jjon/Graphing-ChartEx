#!/usr/bin/python
# -*- coding: utf-8 -*-
## replaced full stops w/unicode bullets, encoded file is UTF-8. Resulting file is the same length, so it shouldn't disturb brat's annotations.
...
def scrape_file(file_path):
    ...
    text = soup.find('div', 'urkundenbox').text.replace('peskyspace', ' ')
    text = re.sub('\s+', ' ', text).strip()
    text = text.replace('.', u'•').encode('utf-8')
...
#     fout = open('/Users/jjc/Documents/ChartEx/initialSets/Cluny/scrapedHTML/cluny-' + str(docNo).rjust(4, '0') + '.html', mode='w')
#     fout.write(doc.read())
#     doc.close()


##### Then Scrape the local html files and write to .txt and .ann files #############
scrapedPages = [os.path.join('/Users/jjc/Documents/ChartEx/initialSets/Cluny/scrapedHTML/', f) for f in os.listdir('/Users/jjc/Documents/ChartEx/initialSets/Cluny/scrapedHTML/')][1:]

for page in scrapedPages:
    docid, title, dortext, text = scrape_file(page)
    txt = file("/Users/jjc/Documents/ChartEx/initialSets/Cluny/markup-set/" + docid + ".txt", mode='w')
    ann = file("/Users/jjc/Documents/ChartEx/initialSets/Cluny/markup-set/" + docid + ".ann", mode='w')
